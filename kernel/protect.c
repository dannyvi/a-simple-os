...
// 全部初始化为中断门（没有陷阱门）
	...
	init_idt_desc(	INT_VECTOR_BREAKPOINT,	DA_386IGATE,
									breakpoint_exception,		PRIVILEGE_KRNL);
								
	init_idt_desc(	INT_VECTOR_OVERFLOW,		DA_386IGATE,
									overflow,								PRIVILEGE_KRNL);
	...
	init_desc(&gdt[INDEX_TSS],
									v2p(s2p(SELECTOR_KERNEL_DS),&tss),
									sizeof(tss)-1,
									DA_386TSS);
	tss.iobase = sizeof(tss);	 /** none */
